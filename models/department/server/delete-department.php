<?php
    ...
    if(isset($_POST["department-id"]) && isset($_POST["_token"]) && isset($_SESSION["token"]) && isset($_POST["soft-delete"])){
        ...
        if(time() > $_SESSION["tokenexpire"]){
            ...
            if($_SESSION["token"] ==! $formToken){
                ...
                if(isset($soft_delete) &&  $soft_delete === "1"){
                    ...
                    $DeleteDepartment = $DepartmentObject->DeleteDepartmentData($department_id);

                }
                else{

                    $DeleteDepartment = $DepartmentObject->SoftDeleteDepartmentData($department_id);

                }

                $response = json_decode($DeleteDepartment);
            }
        }
